...
type
  TFramePayOrderCommon = class(TFrame)
...
var
  GlobalPayOrderCommonFrame:TFramePayOrderCommon;
...
procedure TFramePayOrderCommon.btnOKClick(Sender: TObject);
begin
  HideVirtualKeyboard;
...
procedure TFramePayOrderCommon.btnReturnClick(Sender: TObject);
begin
...
constructor TFramePayOrderCommon.Create(AOwner: TComponent);
begin
  inherited;
...
destructor TFramePayOrderCommon.Destroy;
begin
//  FreeAndNil(FOrder);
...
procedure TFramePayOrderCommon.DoGetAppPayTypeListExecute(ATimerTask: TObject);
begin
...
procedure TFramePayOrderCommon.DoGetAppPayTypeListExecuteEnd(ATimerTask: TObject);
var
...
procedure TFramePayOrderCommon.DoGetChargeFreeExecute(ATimerTask: TObject);
begin
//出错
  TTimerTask(ATimerTask).TaskTag:=1;
...
procedure TFramePayOrderCommon.DoGetChargeFreeExecuteEnd(ATimerTask: TObject);
var
  ASuperObject:ISuperObject;
begin
...
procedure TFramePayOrderCommon.DoGetUserWalletExecute(ATimerTask: TObject);
begin
  //出错
  TTimerTask(ATimerTask).TaskTag:=1;
...
procedure TFramePayOrderCommon.DoGetUserWalletExecuteEnd(ATimerTask: TObject);
var
  ASuperObject:ISuperObject;
...
procedure TFramePayOrderCommon.Do_AlipayMobilePay_OnPayResult(Sender: TObject;
                                     APayResultJson:String;
                                     APayResultStatus:String;
                                     APayResultDesc:String;
                                     APayResultMemo:String;
...
procedure TFramePayOrderCommon.Do_WeiChat_PayResult(Sender: TObject;
                              AResponseCode:Integer;
                              AIsSucc:Boolean;
                              AError:String);
begin
...
procedure TFramePayOrderCommon.GetUserMoney;
begin
  uTimerTask.GetGlobalTimerThread.RunTempTask(
...
procedure TFramePayOrderCommon.lbPaymentTypeListClickItem(AItem: TSkinItem);
begin
//  ShowWaitingFrame(Self,'加载中...');
  //调用获取手续费接口
//  uTimerTask.GetGlobalTimerThread.RunTempTask(
...
procedure TFramePayOrderCommon.Load(//AOrder:TOrder;
                    AOrderFID:Integer;
                    AOrderType:String;
                    ASumMoney:Double;
                    ABillCode:String;
...
begin
  ...
  FSumMoney:=ASumMoney;
  FBillCode:=ABillCode;
  FOrderCaption:=AOrderCaption;
  FOrderDesc:=AOrderDesc;
  FIsNeedSeeOrderButton:=AIsNeedSeeOrderButton;
...
procedure TFramePayOrderCommon.OnModalResultFromCancelPay(Frame: TObject);
begin
...
procedure TFramePayOrderCommon.tteSimulatorOrderAlipaySuccBegin(
...
procedure TFramePayOrderCommon.tteSimulatorOrderAlipaySuccExecute(ATimerTask: TTimerTask);
begin
  //出错
  TTimerTask(ATimerTask).TaskTag:=1;
  try
...
procedure TFramePayOrderCommon.tteSimulatorOrderAlipaySuccExecuteEnd(
  ATimerTask: TTimerTask);
var
  ASuperObject:ISuperObject;
begin
...
procedure TFramePayOrderCommon.tteSimulatorOrderPxpaySuccBegin(
  ATimerTask: TTimerTask);
begin
  ShowWaitingFrame(Self,'Pxpay支付中...');
end;

procedure TFramePayOrderCommon.tteSimulatorOrderPxpaySuccExecute(
  ATimerTask: TTimerTask);
begin
  //出错
  TTimerTask(ATimerTask).TaskTag:=1;
...
procedure TFramePayOrderCommon.tteSimulatorOrderPxpaySuccExecuteEnd(
  ATimerTask: TTimerTask);
var
  ASuperObject:ISuperObject;
begin
...
procedure TFramePayOrderCommon.tteSimulatorOrderWxpaySuccBegin(
  ATimerTask: TTimerTask);
begin
  ShowWaitingFrame(Self,'微信支付中...');
end;

procedure TFramePayOrderCommon.tteSimulatorOrderWxpaySuccExecute(
  ATimerTask: TTimerTask);
begin
  //出错
  TTimerTask(ATimerTask).TaskTag:=1;
...
procedure TFramePayOrderCommon.tteSimulatorOrderWxpaySuccExecuteEnd(
  ATimerTask: TTimerTask);
var
  ASuperObject:ISuperObject;
begin
...
procedure TFramePayOrderCommon.tmrGetOrderPayStateTimer(Sender: TObject);
begin
...
procedure TFramePayOrderCommon.tteGetOrderPayStateExecute(ATimerTask: TTimerTask);
begin
  //出错
  TTimerTask(ATimerTask).TaskTag:=1;
  try
    ...
    TTimerTask(ATimerTask).TaskDesc:=
        SimpleCallAPI('get_bill_money_info',
                      nil,
                      PayCenterInterfaceUrl,
                      ['appid',
                      'user_fid',
...
procedure TFramePayOrderCommon.tteGetOrderPayStateExecuteEnd(ATimerTask: TTimerTask);
var
  ASuperObject:ISuperObject;
  APayState:String;
  ...
  try
    ...
    begin
        ...
        begin
            ...
            APayState:=ASuperObject.O['Data'].S['pay_state'];
            AClientBearPayFee:=ASuperObject.O['Data'].F['client_bear_pay_fee'];
...
//            uBaseLog.OutputDebugString('OrangeUI TFramePayOrder.tteGetOrderPayStateExecuteEnd PayState '+FOrder.pay_state);
            ...
            begin
                  ...
                  if (APayState=Const_PayState_Payed) then
                  begin
                      //付款成功
                      HideWaitingFrame;
...
//                      GlobalPayOrderResultFrame.FrameHistroy:=CurrentFrameHistroy;
                      GlobalPayOrderResultFrame.Load(FSumMoney,
                                                AClientBearPayFee,
                                                Self.lbPaymentTypeList.Prop.SelectedItem.Name,
                                                Const_PayState_Payed,
                                                '付款成功',
                                                '',
                                                FIsNeedSeeOrderButton);
                      FGetOrderPayStateIsOver:=True;

//                      //下完单后刷新我的界面 余额 积分 等数据
//                      if GlobalMainFrame.FMyFrame<>nil then
//                      begin
//                        if FPayType=Const_PaymentType_AccountBalance then GlobalMainFrame.FMyFrame.GetWallet;
//                        GlobalMainFrame.FMyFrame.GetUserInfo;
//                      end;
                  end
                  else if (APayState=Const_PayState_PayTillDone) then
                  begin
                      HideWaitingFrame;
...
procedure TFramePayOrderCommon.ttePayByAccountBalanceBegin(ATimerTask: TTimerTask);
begin
  ShowWaitingFrame(Self,'余额支付中...');
end;

procedure TFramePayOrderCommon.ttePayByAccountBalanceExecute(ATimerTask: TTimerTask);
begin
  //出错
  TTimerTask(ATimerTask).TaskTag:=1;
  try
...
procedure TFramePayOrderCommon.ttePayByAccountBalanceExecuteEnd(
  ATimerTask: TTimerTask);
var
  ASuperObject:ISuperObject;
begin
...
procedure TFramePayOrderCommon.tteUserPrePayOrderBegin(Sender: TTimerTask);
begin
  ShowWaitingFrame(Self,'加载中...');
end;

procedure TFramePayOrderCommon.tteUserPrePayOrderExecute(ATimerTask: TTimerTask);
begin
  //出错
  TTimerTask(ATimerTask).TaskTag:=1;
  try
    TTimerTask(ATimerTask).TaskDesc:=
        SimpleCallAPI('prepare_pay_order',
                      ...
                      'order_fid',
                      'money',
                      'order_type',//商家中心/配送中心的订单:shop_center/delivery_center
                      'bill_code',
                      'pay_type',
                      'name',
                      'desc'],
...
procedure TFramePayOrderCommon.tteUserPrePayOrderExecuteEnd(ATimerTask: TTimerTask);
var
  ASuperObject:ISuperObject;
begin
  try
